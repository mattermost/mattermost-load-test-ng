...
func (t *Terraform) initLoadtest(extAgent *ssh.ExtAgent, ip string) error {
	...
	cmd := "cd mattermost-load-test-ng && export PATH=$PATH:/usr/local/go/bin && go run ./cmd/lt-agent init"
...
func (t *Terraform) configureAndRunAgent(extAgent *ssh.ExtAgent, ip string, output *terraformOutput) error {
	...
	cmd := "cd mattermost-load-test-ng && export PATH=$PATH:/usr/local/go/bin && go run ./cmd/lt-agent server"
...
func (t *Terraform) configureAndRunCoordinator(extAgent *ssh.ExtAgent, ip string, output *terraformOutput) error {
	...
	cmd := "cd mattermost-load-test-ng && export PATH=$PATH:/usr/local/go/bin && go build -o lt-coordinator ./cmd/lt-coordinator"
	if out, err := sshc.RunCommand(cmd); err != nil {
