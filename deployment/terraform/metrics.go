...
func (t *Terraform) UploadDashboard(dashboard string) (string, error) {
	...
	url := fmt.Sprintf("http://%s:%s@%s:3000/api/dashboards/db", "admin", t.config.GeneratedValues.GrafanaAdminPassword, output.MetricsServer.GetConnectionIP())
...
func (t *Terraform) setupMetrics(extAgent *ssh.ExtAgent) error {
	...
	password := string(s)
	cmd = "sudo grafana-cli --homepath \"/usr/share/grafana\" admin reset-admin-password " + password
	if out, err := sshc.RunCommand(cmd); err != nil {
		return fmt.Errorf("error running ssh command: cmd: %s, output: %s, err: %v", cmd, out, err)
	}
	t.config.GeneratedValues.GrafanaAdminPassword = password
