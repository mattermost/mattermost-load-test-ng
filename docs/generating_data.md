...
4. Now tweak everything you need in the controller configuration. All the settings are [documented](config/gencontroller.md), but here are some recommendations:
  ...
  - Scheduled-post seeding is opt-in. For example, use `"NumScheduledPosts": 1000` with `"PercentRecurringScheduledPosts": 0.25` to make each row independently have a 25% chance of recurring weekly. The target requires a licensed server with scheduled posts enabled and at least one configured member channel; a non-zero recurring percentage also requires a server version that supports recurring scheduled posts. Seeded rows are randomized approximately 2–12 days into the future so they remain available for the subsequent load test. Recurring rows prefer the user's valid IANA timezone and otherwise use a valid fallback.
