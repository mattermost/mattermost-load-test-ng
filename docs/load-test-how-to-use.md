 ...
 - Some additional information on the above document
    - When performing [this](https://github.com/mattermost/mattermost-load-test-ng/blob/master/docs/terraform_loadtest.md#copy-and-modify-the-required-configuration) step, edit `MattermostLicenseFile` value to the path containing the license.
        ...
        - When there's unmerged changes to `mattermost`, or `mattermost-load-test-ng`, you need to update these values:
            ...
            - **Mattermost Agents + unmerged load-test-ng stack:** Until the Agents `loadtest` integration and matching `mattermost-load-test-ng` branch are merged and released together, Terraform needs both a **custom load-test-ng tarball** (built from this repo with `make package`) **and** the Agents plugin via deployer **`MattermostPlugins`**: include the Agents plugin manifest ID `mattermost-ai` with a reachable Agents plugin tarball URL (or `file://` path). Point `LoadTestDownloadURL` at `file:///absolute/path/to/mattermost-load-test-ng/dist/<version>/mattermost-load-test-ng-<version>-linux-amd64.tar.gz` for local iteration, or upload that tarball and use its HTTPS URL—the packaged tree includes `config/simulcontroller.json` (from [`config/simulcontroller.sample.json`](../config/simulcontroller.sample.json)) so enabled plugins follow the artifact you deploy. Configure the Agents bot for load testing (`loadtest_mock` service type with MCP tools set to **`auto_run_everywhere`**) per Agents setup docs so mention/DM prompts run without blocking on manual tool approval in the terraform environment.
